(ns ietf-rfcs.main
  "Kotodama WASM entrypoint for the ietf_rfcs clean-room actor (L4) — Clojure port.
  ...
  facts (`ietf_rfcs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ietf_rfcs")
(def tier "L4")
...
(def entity-specs
  [{:entity "Work"       :plural "works"       :id-prefix "ietfrfcs_wor"
    :fields [:doi :title :type :publishedAt]   :required [:doi :title]
    :coerce {}                       :refs {}}
   {:entity "Author"     :plural "authors"     :id-prefix "ietfrfcs_aut"
    :fields [:orcid :givenName :familyName]    :required [:orcid :givenName]
    :coerce {}                       :refs {}}
   {:entity "Citation"   :plural "citations"   :id-prefix "ietfrfcs_cit"
    :fields [:workId :citedDoi]                :required [:citedDoi]
    :coerce {}                       :refs {:workId "Work"}}
   {:entity "Standard"   :plural "standards"   :id-prefix "ietfrfcs_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                       :refs {}}
   {:entity "Record"     :plural "records"     :id-prefix "ietfrfcs_rec"
    :fields [:accession :database :title]      :required [:accession :database]
    :coerce {}                       :refs {}}
   {:entity "Subject"    :plural "subjects"    :id-prefix "ietfrfcs_sub"
...
(defn emit-facts
  "EAVT facts for one record: {\"ietf_rfcs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ietf_rfcs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ietf_rfcs-compat/ready)
