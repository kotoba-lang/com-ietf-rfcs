(ns ietf-rfcs.main-test
  "Contract + behavioral test for the ietf_rfcs-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "ietf_rfcs-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'ietf-rfcs.main-test)]
